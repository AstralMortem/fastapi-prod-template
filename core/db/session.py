...
from sqlalchemy.orm import Session, sessionmaker, mapped_column, Mapped, declared_attr, DeclarativeBase
from core.config import settings
from sqlalchemy.ext.asyncio import async_sessionmaker, create_async_engine, AsyncSession, async_scoped_session
from core.utils.typography import pluralize, camel2snake
...
class Model(DeclarativeBase):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if settings.DEFAULT_PK_FIELD_NAME and not hasattr(cls, settings.DEFAULT_PK_FIELD_NAME) and not cls.__without_default_pk__:
            if settings.DEFAULT_PK_FIELD_TYPE == str:
                ...
                column = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
            elif settings.DEFAULT_PK_FIELD_TYPE == datetime:
                column = mapped_column(DateTime, primary_key=True, default=datetime.now(UTC))
            else:
                raise ValueError(f"Invalid DEFAULT_PK_FIELD_TYPE: {settings.DEFAULT_PK_FIELD_TYPE}")

            setattr(cls, settings.DEFAULT_PK_FIELD_NAME, column)
            cls.__annotations__[settings.DEFAULT_PK_FIELD_NAME] = Mapped[settings.DEFAULT_PK_FIELD_TYPE]
